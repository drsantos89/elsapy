...
class TestElsAuthor:
    ...
    def test_read_metrics_new_author(self):
        ...
        assert (
            myAuth.data["coredata"]["citation-count"]
            and myAuth.data["coredata"]["cited-by-count"]
            and myAuth.data["coredata"]["document-count"]
            and myAuth.data["h-index"]
        )

    def test_read_metrics_existing_author(self):
        self.myAuth.read_metrics(self.good_client)
        assert (
            self.myAuth.data["coredata"]["citation-count"]
            and self.myAuth.data["coredata"]["cited-by-count"]
            and self.myAuth.data["coredata"]["document-count"]
            and self.myAuth.data["h-index"]
        )
...
class TestElsAffil:
    ...
    def test_read_docs(self):
        ...
        assert len(self.myAff.doc_list) == int(
            self.myAff.data["coredata"]["document-count"]
        )
...
class TestFullDoc:
    ...
    full_doi_uri = (
        "https://api.elsevier.com/content/article/doi/10.1016/S1525-1578(10)60571-5"
    )
    ...
    def test_write(self):
        ...
        assert util.file_exist_with_id(
            self.myFullDoc.data["coredata"]["pii"]
            .replace("-", "")
            .replace("(", "")
            .replace(")", "")
        )
...
class TestSearch:
    ...
    searches = [
        ElsSearch(search_type["query"], search_type["index"])
    ...
    def test_init_uri(self):
        ...
        for i in range(len(self.search_types)):
            if (
                self.searches[i].query != self.search_types[i]["query"]
                or self.searches[i].index != self.search_types[i]["index"]
                or self.searches[i].uri
                != (
                    self.base_url
